...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Dataset root containing Train/, Validate/, and Test/. In Colab this can point to a cloned repo or Google Drive folder.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=default_root / "outputs_fastvit",
        help="Directory where models, plots, and reports will be written.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="DataLoader workers. Use -1 to choose a Colab-friendly automatic value.",
    )
...
def running_in_colab() -> bool:
    """Detect whether the script is running inside Google Colab."""

    return "COLAB_RELEASE_TAG" in os.environ or "google.colab" in os.environ.get("JPY_PARENT_PID", "")


def default_project_root() -> Path:
    """Choose a practical default root for local runs and notebook environments."""
...
def select_device(requested_device: str) -> torch.device:
    """Pick the requested device or fall back to the best available accelerator."""

    if requested_device != "auto":
        if requested_device == "cuda" and not torch.cuda.is_available():
            raise RuntimeError("CUDA was requested but is not available in this runtime.")
        if requested_device == "mps" and not torch.backends.mps.is_available():
            raise RuntimeError("MPS was requested but is not available in this runtime.")
...
def resolve_num_workers(requested_workers: int, device: torch.device) -> int:
    """Choose a conservative DataLoader worker count that behaves well in Colab."""
...
def validate_data_dir(data_dir: Path) -> None:
    """Fail early with a clear message when the dataset root is not structured as expected."""

    missing = [split for split in ("Train", "Validate", "Test") if not (data_dir / split).exists()]
    if missing:
        raise FileNotFoundError(
            "Dataset root is missing required split folders "
            f"{missing}. Expected structure: {data_dir}/Train, {data_dir}/Validate, {data_dir}/Test."
        )
...
def main() -> None:
    ...
    print(f"DataLoader workers: {resolve_num_workers(args.num_workers, device)}")
